---
layout: post
title: 2016-12-13-定时器你真的会使用吗？
date: 2016-12-13 11:11:11.000000000 +09:00
tags: loxue
---
# 前言
...
需要**注意**的是： `UIScrollView` 滑动时执行的是 `UITrackingRunLoopMode`，`NSDefaultRunLoopMode`被挂起，会导致定时器失效，等恢复为**滑动结束**时才恢复定时器。
...
CADisplayLink 刷新频率与屏幕帧数相同，用于绘制动画。
